...
export function renderProjects(projects, userName, userRole, expandedState = {}) {
    ...
    // Находим полный объект проекта в хранилище, чтобы получить его ID
    const allProjects = store.getAppData().allProjects || [];
    const fullProject = allProjects.find(p => p.projectName === project.name);

    // Если по какой-то причине проект не найден, пропускаем его
    ...
    // ИСПОЛЬЗУЕМ fullProject.projectId и fullProject.projectName ДЛЯ КОРРЕКТНЫХ ДАННЫХ
    const manageMembersButton = isAdminOrOwner 
        ? `<button class="manage-members-btn p-2 rounded-full absolute top-2 right-2 hover:bg-gray-200 dark:hover:bg-gray-700 z-10" data-project-id="${fullProject.projectId}" data-project-name="${fullProject.projectName}">
    ...
    detailsContainer.innerHTML = `
        <div class="p-4 rounded-lg space-y-4 edit-container">
            <div class="flex justify-between items-start">
                ...
                <button class="delete-btn p-2 rounded-full text-red-500 ml-2">
                ...
                <button class="edit-btn p-2 rounded-full ml-4 flex-shrink-0">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.5L14.732 3.732z"></path>
                    </svg>
                </button>
